#!/usr/bin/env bash
# Causal-JEPA object-centric pre-training launcher.
#
# Usage:
#   ./run_train_CausalJEPA.sh config.yaml
#   NPROC_PER_NODE=4 ./run_train_CausalJEPA.sh config.yaml
#
...
CONFIG="${1:?usage: run_train_CausalJEPA.sh config.yaml}"
...
echo "==> torchrun --standalone --nproc_per_node=$NPROC_PER_NODE train_causal_jepa.py"
echo "    config     = $CONFIG"
