...
from .vision_transformer import PatchEmbed_new, VisionTransformer
...
class SpatialAST(VisionTransformer):
    def __init__(self, num_cls_tokens=3, **kwargs):
        ...
        self.melscale_filterbank = melscale_fbanks(n_freqs=1024,
                                                   f_min=50,
                                                   f_max=14000,
                                                   n_mels=128,
                                                   sr=32000)
    ...
    def random_masking_2d(self, x, mask_t_prob, mask_f_prob):
        ...
        # x_masked = x.masked.reshape(N, len_keep_T*F, D)
    ...
    def forward_features_mask(self, x, mask_t_prob, mask_f_prob):
        ...
        if mask_t_prob > 0.0 or mask_f_prob > 0.0:
            x, mask, id_restore = self.random_masking_2d(x, mask_t_prob, mask_f_prob)
...
def build_AST(**kwargs):
    model = SpatialAST(patch_size=16,
                       embed_dim=768,
                       depth=12,
                       num_heads=12,
                       mlp_ratio=4,
